...
def create_pointset(data, xn, yn, an):
    xm, ym, am = (metrics[xn], metrics[yn], metrics[an])
    rev_y = -1 if ym["worst"] < 0 else 1
    rev_x = -1 if xm["worst"] < 0 else 1
    rev_a = -1 if am["worst"] < 0 else 1
    data.sort(key=lambda t: (rev_y * t[-2], rev_x * t[-3], rev_a * t[-1]))

    axs, ays, als, ays_additional = [], [], [], []
    # Generate Pareto frontier
    xs, ys, ls, ys_additional = [], [], [], []
    last_x = xm["worst"]
    comparator = (lambda xv, lx: xv > lx) if last_x < 0 else (lambda xv, lx: xv < lx)
    for algo, algo_name, xv, yv, yv_additional in data:
        if not xv or not yv or not yv_additional:
            continue
        axs.append(xv)
        ays.append(yv)
        ays_additional.append(yv_additional)
        als.append(algo_name)
        if comparator(xv, last_x):
            last_x = xv
            xs.append(xv)
            ys.append(yv)
            ys_additional.append(yv_additional)
            ls.append(algo_name)
    return xs, ys, ys_additional, ls, axs, ays, ays_additional, als


def compute_metrics(true_nn_distances, res, metric_1, metric_2, metric_3, recompute=False):
    all_results = {}
    for i, (properties, run) in enumerate(res):
        ...
        print("%3d: %80s %12.3f %12.3f %12.3f" % (i, algo_name, metric_1_value, metric_2_value, metric_3_value))
...
def get_plot_label(xm, ym, ym_additional):
    template = "%(xlabel)s-%(ylabel)s-%(ylabel_additional)s tradeoff"
    return template % {
        "xlabel": xm["description"],
        "ylabel": ym["description"],
        "ylabel_additional": ym_additional["description"],
    }
