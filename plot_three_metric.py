...
from ann_benchmarks.plotting.custom_utils_three_metrics import (compute_metrics, create_linestyles,
                                                                create_pointset, get_plot_label)
from ann_benchmarks.results import get_unique_algorithms, load_all_results


def create_plot(all_data, raw, x_scale, y_scale, xn, yn, yn_additional, fn_out, linestyles, batch):
    xm, ym, ym_additional = (metrics[xn], metrics[yn], metrics[yn_additional])
    ...
    def mean_y(algo):
        xs, ys, ys_additional, ls, axs, ays, ays_additional, als = create_pointset(all_data[algo], xn, yn, yn_additional)
        return -np.log(np.array(ys + ys_additional)).mean()
    ...
    for algo in sorted(all_data.keys(), key=mean_y):
        ...
        # Plot third metric on the y-axis
        (handle1,) = plt.plot(
            xs, ys_additional, "--", label=algo, color=faded, ms=7, mew=3, lw=3, marker=marker
        )
    ...
    ax = plt.gca()
    ax.set_ylabel(ym["description"] + " AND " + ym_additional["description"] + " (faded)")
    ...
    ax.set_title(get_plot_label(xm, ym, ym_additional))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-ay", "--additional-y-axis", help="Additional third metric the on y axis"
    )
    ...
    runs = compute_metrics(np.array(dataset["distances"]), results, args.x_axis, args.y_axis, args.additional_y_axis,
                           args.recompute)
    if not runs:
        raise Exception("Nothing to plot")

    create_plot(
        runs, args.raw, args.x_scale, args.y_scale, args.x_axis, args.y_axis, args.additional_y_axis, args.output, linestyles,
        args.batch
    )
